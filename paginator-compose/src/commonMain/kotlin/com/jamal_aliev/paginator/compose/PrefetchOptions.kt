...
/**
 * Bag of runtime-mutable settings for a [PaginatorPrefetchController] / DSL helper.
 *
 * Marked [Immutable] so that the Compose compiler treats it as a stable parameter — passing
 * the same instance across recompositions does not invalidate downstream skipping.
 *
 ...
 * @property cancelOnDispose If `true`, [PaginatorPrefetchController.cancel] runs on `onDispose`.
 *   Set to `false` to let an in-flight prefetch survive a brief composition gap (overlay,
 *   bottom-sheet) so the page lands when the screen returns.
