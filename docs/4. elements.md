...
#### Overflow past the last page: the `initPageState` factory

When the cascade runs past the **last** cached page, the overflow would have nowhere to go. Every
insert that can reach the end — `addElement`, `addAllElements`, `prependElement`, `moveElement`,
`insertBefore`, `insertAfter` — takes a page-factory parameter (`initPageState`, or
`initSuccessPageState` on the single-element appenders) that **defaults** to creating a fresh
trailing page so the overflow is **not dropped**. The new page is the **same class** as the page
that overflowed (a custom `OffsetPageState` subclass is preserved) with a fresh id.

```kotlin
// Default: a full last page → addElement creates a trailing page for the new item.
paginator.addElement(newItem)

// Opt out: pass null and the overflow is dropped instead of growing the cache off-screen.
paginator.addElement(newItem, initSuccessPageState = null)

// Custom: build the trailing page yourself.
paginator.addAllElements(
    elements = listOf(newItem),
    targetPage = 3,
    index = 0,
    initPageState = { page, data -> MySuccessPage(page, data) },
)
```

Pure remove / replace / reorder operations (`setElement`, `removeElement`, `swapElements`,
`removeAll`, `retainAll`, `distinctBy`, `updateAll`, `updateWhere`) never grow the list, so they
take no factory.
