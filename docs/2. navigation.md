...
> Looking for cursor-based navigation (`prev`/`self`/`next` tokens instead of numeric pages)?
> See [Cursor-Based Pagination → Navigation](13.%20cursor-pagination.md#navigation). The method
> names and lock flags are the same; the only differences are end-of-feed semantics
> (`next == null` / `prev == null` → `EndOfCursorFeedException`) and the `bookmark` type
> (`CursorBookmark` rather than `BookmarkInt`).
