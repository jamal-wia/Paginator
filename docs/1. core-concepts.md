...
`PageState<E>` is a **strategy-agnostic interface** representing the state of a single page. It
carries only `data: List<E>`, an optional `metadata`, and a unique `id: Long` — deliberately **no
positional key**, because offset and cursor pagination key pages differently. Load status is
expressed through three nested marker interfaces:

| Marker                       | Description                                                                                                              |
|------------------------------|--------------------------------------------------------------------------------------------------------------------------|
| `PageState.SuccessState<E>`  | Successfully loaded page. `data` may be empty (detect via `isEmptyState()`) or non-empty (detect via `isSuccessState()`) |
| `PageState.ProgressState<E>` | Page currently being loaded. May contain cached data from a previous load                                                |
| `PageState.ErrorState<E>`    | Page that failed to load. Carries the `exception` and may contain previously cached data                                 |

Match on these markers (`is PageState.SuccessState`) or use the `isSuccessState()` /
`isErrorState()`
/ `isProgressState()` predicates when you want to stay strategy-agnostic.

Each strategy provides its own concrete tree implementing these markers:

- **Offset** (`paginator-offset`): `OffsetPageState.{Success, Progress, Error}<T>`, keyed by a
  numeric `page: Int`. Ordered by `compareTo`.
- **Cursor** (`paginator-cursor`): `CursorPageState.{Success, Progress, Error}<K, T>`, keyed by a
  typed `bookmark: CursorBookmark<K>`. Ordered by the cache's `prev`/`next` links.

Both concrete classes are `sealed` within their module but their subclasses are `open`, so you can
...
) : OffsetPageState.Progress<T>(page, data)

// Cursor (note the key type parameter K)
class PercentCursorProgressPage<K : Any, T>(
  bookmark: CursorBookmark<K>,
  data: List<T>,
  val progressPercent: Int = 0,
) : CursorPageState.Progress<K, T>(bookmark, data)
```
