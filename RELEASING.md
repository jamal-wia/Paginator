...
Starting with 10.0.4 the library publishes **eight artifacts** that are released together and must
...
```properties
# gradle.properties
paginator.version=10.0.4   # ← new version (propagates to every module and to the BOM constraints)
```
...
```bash
git add -A
git commit -m "Bump version to 10.0.4"
...
2. Click **"Choose a tag"** and type the new version (e.g. `10.0.4`), then select **"Create new tag
   on publish"**
3. Set **Release title** (e.g. `10.0.4`)
