...
```kotlin
version = "8.6.2" // ← new version (must match in all three modules)
```
...
```bash
git add -A
git commit -m "Bump version to 8.6.2"
...
2. Click **"Choose a tag"** and type the new version (e.g. `8.6.2`), then select **"Create new tag
   on publish"**
3. Set **Release title** (e.g. `8.6.2`)
