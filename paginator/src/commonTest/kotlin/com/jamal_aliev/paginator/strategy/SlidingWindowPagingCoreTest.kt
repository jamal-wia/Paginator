...
class SlidingWindowPagingCoreTest {
    ...
    fun `evicts pages outside context window`() {
        val core = SlidingWindowPagingCore<String>(initialCapacity = 5)
    ...
    fun `margin extends the keep range`() {
        val core = SlidingWindowPagingCore<String>(initialCapacity = 5, margin = 1)
        ...
        // Page 6 is outside (2-1)..(4+1) = 1..5? No, 6 > 5, evicted
        // Wait, we just added page 6, and context is 2..4, margin=1, keep = 1..5
        // Page 6 is outside → immediately evicted? No, we need to check.
        // Actually, the page was just added, so performEviction runs and evicts it.
        // BUT we don't want to evict a page we just added. Let me reconsider...
        // Actually for SlidingWindow, it makes sense: if a page is outside the window, don't keep it.
        // This would only happen if someone manually sets a page outside the window.
        // During normal Paginator navigation, setState is called by goNextPage which also
        // extends the context window, so the page would be in the window.
    }
    ...
    fun `margin 0 keeps only context window pages`() {
        val core = SlidingWindowPagingCore<String>(initialCapacity = 5, margin = 0)
    ...
    fun `jump evicts pages from previous location`() = runTest {
        val core = SlidingWindowPagingCore<String>(initialCapacity = 3)
        ...
        assertNull(paginator.core.getStateOf(3))
    ...
    fun `margin keeps extra pages after jump`() = runTest {
        val core = SlidingWindowPagingCore<String>(initialCapacity = 3, margin = 1)
        val paginator = MutablePaginator(core = core) { page: Int ->
            List(3) { "p${page}_item$it" }
        }
        ...
        assertNotNull(paginator.core.getStateOf(5))
        assertNotNull(paginator.core.getStateOf(6))
        ...
        assertNull(paginator.core.getStateOf(6))
        assertNotNull(paginator.core.getStateOf(20))
    }
