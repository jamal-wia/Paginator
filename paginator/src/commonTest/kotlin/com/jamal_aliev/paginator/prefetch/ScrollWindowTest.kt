...
class ScrollWindowTest {
    ...
    fun `identity remap when no prefix or suffix offset`() {
        val r = ScrollWindow.from(
            firstVisibleIndex = 3,
            lastVisibleIndex = 8,
            dataItemCount = 10,
            dataOffset = 0,
        )
        assertEquals(ScrollWindow(firstVisibleIndex = 3, lastVisibleIndex = 8), r)
    }
    ...
    fun `shifts indices down by data offset`() {
        ...
        assertEquals(ScrollWindow(firstVisibleIndex = 3, lastVisibleIndex = 7), r)
    }
    ...
    fun `clamps first to zero when viewport spans prefix into data`() {
        ...
        assertEquals(ScrollWindow(firstVisibleIndex = 0, lastVisibleIndex = 2), r)
    }
    ...
    fun `clamps last to data end when viewport spans data into suffix`() {
        ...
        assertEquals(ScrollWindow(firstVisibleIndex = 8, lastVisibleIndex = 9), r)
    }
    ...
    fun `clamps both ends when viewport contains everything`() {
        ...
        assertEquals(ScrollWindow(firstVisibleIndex = 0, lastVisibleIndex = 9), r)
    }
    ...
    fun `single visible data item with prefix offset`() {
        ...
        assertEquals(ScrollWindow(firstVisibleIndex = 3, lastVisibleIndex = 3), r)
    }
}
