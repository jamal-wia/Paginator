 ...
 * Realm collection, WebSocket stream, …) as a stream of [ReactiveEvent]s the
 * paginator can consume.
 *
 ...
 * `MutablePaginator.observe(scope, cache)`; every emission flows through the
 * matching CRUD operation on the paginator's L1 cache without re-entering
 ...
 *    whose `equals` result changes) makes [ReactiveEvent.Updated] and
 *    [ReactiveEvent.Removed] silently fail to locate the cached item.
 *
 * 2. **Hot subscription.** [changes] is collected by the paginator from the
 *    moment [com.jamal_aliev.paginator.extension.observe] is called until the
 *    returned `Job` is cancelled. Returning a cold flow that emits a finite
 *    set and completes is allowed but unusual — once it completes, no further
 *    updates flow to the paginator.
 *
 * 3. **Position correctness for inserts.** When emitting
 *    [ReactiveEvent.Inserted], the [InsertPosition] must reflect the source's
 *    actual ordering. If the position cannot be computed (e.g., a complex
 *    `ORDER BY` that the adapter cannot mirror), prefer
 *    [ReactiveEvent.Invalidate] over guessing.
 *
 * ## Identity type [ID]
 *
 ...
 * ## Initial sync
 *
 * Between the first `load(page)` returning and the paginator subscribing to
 * [changes], events may be lost. The `observe` extension offers an
 * `InitialSyncPolicy` that issues a `refreshAll()` after subscribe to
 * reconcile — see that extension for details.
 *
 ...
 * makes [com.jamal_aliev.paginator.cache.persistent.PersistentPagingCache] (L2)
 ...
 *   `data class`.
 */
interface PaginatorReactiveCache<T, ID : Any> {

    /**
     * Returns a stable domain identity for [item].
     *
     * Used by the paginator to locate cached items when applying
     * [ReactiveEvent.Updated], [ReactiveEvent.Removed], [ReactiveEvent.Moved],
     * and the `*Identity` variants of [InsertPosition]. The returned value
     * must:
     ...
     */
    fun changes(): Flow<ReactiveEvent<T, ID>>
}
