...
- **Тихая подгрузка (opt-in).** Установите `silentlyLoading = true`, чтобы подавить эмиссию
  `ProgressPage` snapshot во время prefetch — удобно для чат-ленты, где не хочется, чтобы
  спиннер мигал при каждом подлёте к краю. По умолчанию — `false`, чтобы append-индикатор,
  привязанный к `PaginatorUiState.Content.appendState`, отображался автоматически.
...
А когда пользователь скроллит *вверх* за старыми сообщениями, только что подгруженная страница не
должна дёргать текущий вид. Рендерите индикатор «загрузка предыдущей» **вне** списка — через
`PaginatorScrollEdgeIndicators` или turnkey-composable `PaginatedLazyColumn` из
`paginator-compose-offset` / `paginator-compose-cursor` — и список сохраняет позицию: старая
страница вставляется над кромкой экрана, а не выбрасывает вас в её начало. В Paging 3 это тот самый
пресловутый prepend-прыжок, с которым воюешь через scroll anchoring в `RecyclerView`; здесь — одна
composable-обёртка.
...
2. Page=1 уже содержит `capacity=50` элементов — значит, после вставки их стало 51. Переполнение
   ...
   `capacity` элементов» держится автоматически. Когда каскад доходит до **последней** закэшированной
   страницы, по умолчанию создаётся новая страница в хвосте — чтобы ничего не терялось. Для ленты,
   где низ это всё равно история, которую вы перезагрузите, передайте `initPageState = null`, чтобы
   вместо роста кэша отбрасывать самый старый элемент за кадром.
