...
трогать. Основано на диффе между коммитами `f58bf336` (3.3.0) и `c8389580` (8.6.2).
...
## 5. `Paginator` против `MutablePaginator`
...
сценариев можно оставить `Paginator<T>` и получить более узкий API в подарок.

## 6. Партия переименований из 5.0.0

Бо́льшая часть ошибок компиляции после апгрейда придёт именно отсюда (коммит `13aa2bf`):
...
`cache: PagingCache`. Если в 3.3.0 вы лазили в внутреннюю мапу — теперь только через API
`core`/`cache`.
