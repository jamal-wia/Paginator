 ...
 * [com.jamal_aliev.paginator.core.cache.reactive.SnapshotReactiveCache].
 *
 * A ready-made [CursorPaginatorReactiveCache] that turns a snapshot stream
 * (`Flow<List<T>>`) into [CursorReactiveEvent]s by diffing consecutive
 * emissions — the paginator analogue of RecyclerView's `ListAdapter` /
 * `DiffUtil`.
 *
 ...
 * matching [CursorReactiveEvent]s, wrapped in a single [CursorReactiveEvent.Batch]
 * so the change applies atomically.
 *
 * The diffing behaviour is identical to the offset variant; only the emitted
 * event and position types differ ([CursorReactiveEvent] / [CursorInsertPosition]
 * instead of `ReactiveEvent` / `InsertPosition`). [ReorderPolicy] is shared with
 * the offset paginator (it lives in `paginator-core`).
 *
 ...
 * the item at index 0 lands at [CursorInsertPosition.Head], every other new
 * item at [CursorInsertPosition.AfterIdentity] of the preceding item. Walking
 * the snapshot left-to-right and applying events in order reconstructs the
 * target ordering.
 *
 * ## Windowing caveat
 *
 ...
 * under `RefreshAll`). Have the source return only the visible slice when this
 * matters.
 *
 * ## Identity uniqueness
 *
 ...
 *   same content. When it returns `false`, a [CursorReactiveEvent.Updated] is emitted.
 ...
 *   neighbour in the new snapshot (`null` when the item is first). Return a
 *   [CursorInsertPosition], or `null` to **skip** the insert entirely (a
 *   deliberate veto, not an error — the active `UnknownItemPolicy` is not
 *   consulted). The default reproduces the snapshot order:
 *   [CursorInsertPosition.Head] for the first item,
 *   [CursorInsertPosition.AfterIdentity] of the left neighbour otherwise.
 ...
 *     `{ _, _ -> CursorInsertPosition.Head }` (or simpler, reverse the snapshot
 *     list upstream so the default works for batch inserts too);
 *   - conditional insert: `{ item, _ -> if (canPlace(item)) CursorInsertPosition.Head else null }`.
 *
 * @see cursorSnapshotReactiveCache for a factory with an ergonomic `identity =` parameter.
 */
class CursorSnapshotReactiveCache<T, ID : Any>(
    ...
    private val resolveInsert: (item: T, leftNeighborId: ID?) -> CursorInsertPosition<ID>? =
        { _, leftNeighborId ->
            if (leftNeighborId == null) CursorInsertPosition.Head
            else CursorInsertPosition.AfterIdentity(leftNeighborId)
        },
) : CursorPaginatorReactiveCache<T, ID> {

    override fun identity(item: T): ID = identitySelector(item)

    override fun changes(): Flow<CursorReactiveEvent<T, ID>> = flow {
     ...
     * thing changed, or a [CursorReactiveEvent.Batch] (applied in one
     * transaction) when several things changed.
     *
     ...
     */
    private fun diff(previous: List<T>, current: List<T>): CursorReactiveEvent<T, ID>? {
        ...
        val events = ArrayList<CursorReactiveEvent<T, ID>>()
        ...
        for (id in previousById.keys) {
            if (id !in currentIds) events.add(CursorReactiveEvent.Removed(id))
        }
        ...
        if (reordered && reorderPolicy == ReorderPolicy.Invalidate) {
            ...
            return CursorReactiveEvent.Invalidate
        }
        ...
        for (item in current) {
            ...
            if (before == null) {
                ...
                if (position != null) {
                    events.add(CursorReactiveEvent.Inserted(item, position))
                    leftId = id
                }
                continue
            }
            ...
            if (id in movedIds) {
                ...
                events.add(CursorReactiveEvent.Moved(item, positionFor(leftId)))
            } else if (!areContentsTheSame(before, item)) {
                // Same position, but the fields changed → in-place update.
                events.add(CursorReactiveEvent.Updated(item))
            }
        ...
        return when (events.size) {
            0 -> null
            1 -> events[0]
            else -> CursorReactiveEvent.Batch(events)
        }
    }

    /** Head when there is no left neighbour (the first row), else right after it. */
    private fun positionFor(leftId: ID?): CursorInsertPosition<ID> =
        if (leftId == null) CursorInsertPosition.Head else CursorInsertPosition.AfterIdentity(leftId)
...
/**
 * Creates a [CursorSnapshotReactiveCache] — the cursor-paginator analogue of
 * `ListAdapter` / `DiffUtil`. Diffs consecutive emissions of [snapshots] into
 * [CursorReactiveEvent]s so the adapter author writes only an [identity]
 * selector and a snapshot source.
 *
 * ```kotlin
 ...
 * ```
 *
 * See [CursorSnapshotReactiveCache] for the diffing semantics, the
 * first-emission baseline, the windowing caveat, and [ReorderPolicy].
 *
 * @param snapshots The reactive source emitting the full current list on every change.
 * @param identity Returns the stable identity of an item (its primary key).
 * @param areContentsTheSame Decides whether two items with the same identity carry the
 *   same content; `false` triggers a [CursorReactiveEvent.Updated]. Defaults to `old == new`.
 ...
 *   insert. See [CursorSnapshotReactiveCache] for the semantics and common overrides.
 *   Defaults to reproducing the snapshot order.
 */
fun <T, ID : Any> cursorSnapshotReactiveCache(
    ...
    resolveInsert: (item: T, leftNeighborId: ID?) -> CursorInsertPosition<ID>? =
        { _, leftNeighborId ->
            if (leftNeighborId == null) CursorInsertPosition.Head
            else CursorInsertPosition.AfterIdentity(leftNeighborId)
        },
): CursorPaginatorReactiveCache<T, ID> =
    CursorSnapshotReactiveCache(
        snapshots = snapshots,
